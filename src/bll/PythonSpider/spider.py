...
def get_content(id, data_base):
    ...
    # 判断注册时间
    if registration_date > '2019-11-01':
        try:
...
def insert_data(txt_match, id, data_base):
    ...
    if len(location) > 0:
        sql = "INSERT INTO missing_people_data_table VALUES(" + \
                      ...
                      str(location[1]) + ',' + str(location[0]) + ',' + str(1) + ',' + str(0) + ',' +'"admin"' + ");"
        # print(sql)
        cursor.execute(sql)
        data_base.commit()
    else:
...
if __name__ == '__main__':
    os.chdir("E://实习//GIS工程//项目参考资料//GoHome-master//GoHome-master//web//libs//image//all_missing_people_photo")
    ...
    cursor.execute("DROP TABLE IF EXISTS missing_people_data_table")

    # 创建数据表SQL语句
    sql = """CREATE TABLE missing_people_data_table (
             missingID  INT(10) NOT NULL,
             type  VARCHAR(11) NOT NULL,
             missingName  VARCHAR(100),
             sex VARCHAR(3),  
             birthDate DATE,
             height VARCHAR(20),
             missingDate DATE,
             missingPlace TEXT(500),
